...
class VideoProcessor:
    ...
    def loadClusterClips(self, allClips = True, mlClips = False):
        if allClips:
            ...
            subprocess.call(['tar', '-C', self.localClusterDirectory, '-xvf', self.localAllClipsDirectory[:-1] + '.tar'], stderr = self.fnull, stdout = , stderr = self.fnull)
        if mlClips:
